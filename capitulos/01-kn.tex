...
\section{Combinaciones lineales y familias generadoras}
	...
	\end{definicion}
	\begin{definicion}
		Una familia de vectores de longitud $m$ en $K^n$ es una $m$-tupla $(v_1,...,v_m)$ donde $v_1,...,v_m\in K^n$.
	\end{definicion}
	\begin{notacion}
		Escribiremos $v_1,...,v_m$ en lugar de $(v_1,...,v_m)$.
	\end{notacion}
	\begin{definicion}
		...
		\end{center}
		se llama generador de $v_1,...,v_m$. Si $S=\gen(v_1,...,v_m)$ se dice que la familia $v_1,...,v_m$ genera $S$, o que $v_1,...,v_m$ es una familia generadora de $S$.
	\end{definicion}
	\begin{proposicion}
...
\section{Independencia lineal en $K^n$}
	\begin{definicion}
		Una familia $v_1,...,v_m$ de vectores en $K^n$ es linealmente dependiente (o ligada) si existen $t_1,...,t_m\in K$ con algún $t_i\neq0$, tales que
		\begin{center}
			$t_1v_1+\cdots+t_mv_m=0$.
		\end{center}
		Una familia $v_1,...,v_m$ de vectores en $K^n$ es linealmente independiente (o libre) si no es linealmente dependiente, es decir, si para todos los $t_1,...,t_m\in K$ tales que
		\begin{center}
			$t_1v_1+\cdots+t_mv_m=0$
		\end{center}
	...
	\begin{observacion}
		\begin{enumerate}
			\item La familia vacía es libre por vacuidad.
			\item Toda familia que contiene a una familia ligada es ligada.
			\item Toda familia contenida en una familia libre es libre.
			\item En una familia libre no puede estar el vector nulo, ni pueden haber vectores repetidos.
		\end{enumerate}
	\end{observacion}
	\begin{proposicion}
		La familia $e_1,...,e_n$ de vectores en $K^n$ es libre.
	\end{proposicion}
	\begin{proposicion}
		Una familia $v_1,...,v_m$ de vectores en $K^n$ es linealmente dependiente si y solo si existe $i\in\{1,...,m\}$ tal que $v_i$ es combinación lineal de los anteriores.
	\end{proposicion}
	\begin{observacion}
		Si una familia de vectores genera un espacio y está familia es ligada, entonces hay un vector que es combinación lineal de los anteriores, si lo suprimimos la familia que resulta genera el mismo espacio.
	\end{observacion}
	\begin{lema}
		Una familia $v_1,...,v_m$ de vectores en $K^n$ es linealmente independiente si y solo si todo vector $v\in\gen(v_1,...,v_m)$ se puede escribir de manera única como combinación lineal de $v_1,...,v_m$.
	\end{lema}
	\begin{teorema}
